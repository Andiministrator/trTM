...
trTM.f.consent_check = function (action) {
  ...
  trTM.d.consent.interactions = cData.customPayload.Interaction;
  ...
  trTM.d.consent.consent_model = obj.ConsentModel.Name;
  if (typeof obj.cctId=='string') trTM.d.consent.consent_id = obj.cctId;
  if (typeof cData.dsDataElements=='object' && typeof cData.dsDataElements.InteractionType=='string') trTM.d.consent.interaction_type = cData.dsDataElements.InteractionType;
  if (typeof cData.dsDataElements=='object' && typeof cData.dsDataElements.Country=='string') trTM.d.consent.country = cData.dsDataElements.Country;
  ...
  trTM.d.consent.purposeIDs = ',' + purpose_ids.join(',') + ',';
  trTM.d.consent.purposes = ',' + purpose_names.join(',') + ',';

  // Build Feedback
  if (trTM.d.consent.interaction_type) { trTM.d.consent.feedback = trConfig.cmp.interaction_type; }
  else if (cats_total==cats_essential_count) { trTM.d.consent.feedback = 'No OptIn Categories available'; }
  else if (cats_count==cats_total) { trTM.d.consent.feedback = 'Consent full accepted'; }
  else if (cats_count==cats_essential_count) { trTM.d.consent.feedback = 'Consent declined'; }
  else if (cats_count<cats_total) { trTM.d.consent.feedback = 'Consent partially accepted'; }
  else { trTM.d.consent.feedback = 'Consent given'; }
